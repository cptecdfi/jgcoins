...
*Setup instructions for a gitian build of Jgcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Jgcoin
...
- [Building Jgcoin](#building-jgcoin)
...
Clone the git repositories for jgcoin and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/jgcoin/jgcoin
```
...
In [doc/release-process.md](release-process.md) in the jgcoin repository under 'Fetch and build inputs'.
...
To build Jgcoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the jgcoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/jgcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/jgcoin/jgcoin
...
For example:
```bash
URL=https://github.com/laanwj/jgcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit jgcoin=${COMMIT} --url jgcoin=${URL} ../jgcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit jgcoin=${COMMIT} --url jgcoin=${URL} ../jgcoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/jgcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/jgcoin-build.assert
```
...
[jgcoin/gitian.sigs](https://github.com/jgcoin/gitian.sigs/) repository, or
