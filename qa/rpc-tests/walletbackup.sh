...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} jgc"
...
function StartThree {
  $JGCOIND $JGCOINDARGS $B1ARGS &
  B1PID=$!
  $JGCOIND $JGCOINDARGS $B2ARGS &
  B2PID=$!
  $JGCOIND $JGCOINDARGS $B3ARGS &
  B3PID=$!
}
